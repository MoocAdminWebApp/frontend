...
const ChapterPage: React.FC<ChapterPageProps> = ({ courseId }) => {
  ...
  const [currentChapter, setCurrentChapter] =
    React.useState<UpdateChapterDto | null>(null);
  ...
  React.useEffect(() => {
    const load = async () => {
      setLoading(true);
      const resp = await get<PagedResultDto<ChapterDto>>(`/chapters/page`, {
        page: filter.page,
  ...
  const handleSave = async (
    chapter: CreateChapterDto | UpdateChapterDto | null
  ) => {
    if (!chapter) return;
    let resp;
    const payload = {
    ...
    if ((chapter as UpdateChapterDto).id) {
      resp = await put(
        `/chapters/${(chapter as UpdateChapterDto).id}`,
        payload
      );
    } else {
    ...
    if (resp.isSuccess) {
      toast.success(
        (chapter as UpdateChapterDto).id
